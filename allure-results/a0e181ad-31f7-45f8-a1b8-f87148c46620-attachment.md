...
- Name: ProductList.spec.ts >> TTA Cart - Product List Page Tests >> CART-002 - Verify shopping cart item count after adding a product | Expected: The cart item count should increment and reflect the added product.
- Location: src\tests\ProductList.spec.ts:130:9
...
Call log:
  - waiting for locator('[data-testid="inventory-item"]').first().getByRole('button', { name: 'Add to cart' })
