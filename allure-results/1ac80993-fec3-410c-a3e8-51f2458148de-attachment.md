...
- Name: ProductList.spec.ts >> TTA Cart - Product List Page Tests >> SORT-001 - Verify default product sorting by name in ascending alphabetical order (A to Z) | Expected: Name (A to Z) should be selected by default and names should be sorted alphabetically.
- Location: src\tests\ProductList.spec.ts:84:9
